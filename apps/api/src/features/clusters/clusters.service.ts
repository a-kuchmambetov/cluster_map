...
function readConfigFile(): ClusterConfig[] {
  ...
  if (!result.success) {
    throw AppError.internal(
      "Cluster layout config failed schema validation",
      result.error.issues,
    );
  }
...
export function loadClusterConfig(clusterNumber: number): ClusterConfig {
  const cluster = readConfigFile().find(
    (config) => config.number === clusterNumber,
  );
...
export async function getClusterOccupancyData(
...
export async function getClusterConfigValidation(
  clusterNumber: number,
): Promise<ConfigValidationResponse> {
  const configs = listClusterConfigs();
  const clusterIndex = configs.findIndex(
    (config) => config.number === clusterNumber,
  );
