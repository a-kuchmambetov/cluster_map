...
import { getClusterConfigValidation, getClusterLayout, getClusterOccupancyData, listClusterConfigs, loadClusterConfig } from "./clusters.service";
...
export function getClusterEventsHandler(req: Request, res: Response, next: NextFunction): void {
    try {
        const { clusterNumber } = clusterNumberParamSchema.parse(req.params);
