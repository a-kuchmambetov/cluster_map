      ...
      {avatars.slice(0, avatarSlots.length).map((avatar, i) => {
        ...
        return (
          <div
            key={i}
            className={cx(
              "absolute top-1/2 left-1/2 rounded-full bg-primary p-px shadow-xs ring-[0.5px] ring-black/10",
              slot.size,
            )}
            style={{
...
const AvatarGrid = ({ avatars = [], ...props }: AvatarGridProps) => {
  ...
  const renderGridAvatar = (
    ...
    <div
      key={key}
      className={cx(
        "shrink-0 bg-primary p-px shadow-xs ring-[0.75px] ring-black/10",
        config.avatar,
      )}
    >
      <div
  ...
  return (
    ...
    >
      <div className="flex">
        <div
          className={cx(
            "flex w-auto max-w-none shrink-0 animate-marquee [animation-duration:240s] motion-reduce:animate-none",
            config.gap,
          )}
        >
