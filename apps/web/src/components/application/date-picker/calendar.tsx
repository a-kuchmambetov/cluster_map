...
export const CalendarContextProvider = ({ children }: PropsWithChildren) => {
    ...
    const isDateValue = (nextValue: DateValue | readonly DateValue[]): nextValue is DateValue => !Array.isArray(nextValue);
    const onChange = (nextValue: DateValue | readonly DateValue[]) => {
        if (!isDateValue(nextValue)) return;
